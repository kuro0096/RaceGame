...
namespace UnityStandardAssets.Cameras
{
    ...
    {
        [SerializeField] private float m_MoveSpeed = 3; // cameraのボーンが動くスピード
        [SerializeField] private float m_TurnSpeed = 1; // 車がターンするときのcameraの速さ
        [SerializeField] private float m_RollSpeed = 0.2f;// cameraの時点の速さ
        [SerializeField] private bool m_FollowVelocity = true;// Whether the rig will rotate in the direction of the target's velocity.
        [SerializeField] private bool m_FollowTilt = false; // Whether the rig will tilt (around X axis) with the target.
        [SerializeField] private float m_SpinTurnLimit = 90;// ターゲットの回転に続いてカメラが停止するしきい値 (used in situations where a car spins out, for example)
